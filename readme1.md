...
export default function ThermalComfortCalculator() {
  // Koefisien model Bab 4 (sesuai dokumen)
  const PARAMS = {
    alpha: 0.225,
    beta1: 0.0774,
    beta2: 7.379,
  ...
  // ✅ Nilai contoh (agar hasil PRE ~2.955 dan NORM mengikuti setting)
  ...
  // Round biasa (untuk tampilan langkah)
  const round3 = (x: number): number => Number(x.toFixed(3))

  // ✅ FIX UTAMA: potong ke 3 desimal (floor) untuk komponen hasil perkalian
  // Ini yang bikin 7.379*0.469 jadi 3.460 (bukan 3.461) seperti dokumen.
  const floor3 = (x: number): number => {
    const eps = 1e-12
    return Math.floor((x + eps) * 1000) / 1000
  }

  /**
   * PMVpesisir (sesuai model dokumen):
   *
   * PMV_pre =
   *   α + β₁·PMViso
   *   + β₂·e^{-k( H/W + αᵥ·v )}
  ...
  const calculatePMVAbran = (
    ...
    // === LANGKAH ===
    const A_full = h_w + PARAMS.alphaV * v
    const exponent_full = -PARAMS.k * A_full
    const exp_raw_full = Math.exp(exponent_full)

    // Dokumen menampilkan expfactor 3 desimal (contoh 0.469)
    const exp_used = round3(exp_raw_full)

    // Dokumen juga memakai SVF*v (3 desimal)
    const SVFv = round3(svf * v)

    // === KOMPONEN (FIX: floor untuk hasil perkalian agar sama persis dokumen) ===
    const termAlpha = round3(PARAMS.alpha)
    const termBeta1 = floor3(PARAMS.beta1 * pmv_iso)
    const termBeta2 = floor3(PARAMS.beta2 * exp_used)     // ✅ FIX UTAMA
    const termBeta3 = floor3(PARAMS.beta3 * SVFv)
    const termBeta4 = floor3(PARAMS.beta4 * veg_func)
    ...
    if (!normalize) {
      return {
        A: round3(A_full),
        exponent: round3(exponent_full),
        expfactor_raw: round3(exp_raw_full),
    ...
    const scale_used = round3(scale_raw) // ditampilkan 3 desimal seperti di web kamu
    const total = round3(pmv_obs_ref * scale_used) // hasil PMV_norm (contoh -3.467)
            ...
            <CardContent className="pt-6 space-y-6">
              ...
              <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  {/* PMV Referensi tetap dipakai dari state, meski inputnya kamu sembunyikan */}
                </div>
              </div>
